...
def run():
    ...
    yolact = YolactFacade()
    stereo_vision = StereoVision(calibrate_stereo_matching=True)

    cap = camera.get_video_live()
    while True:
        _, img_double = cap.read()
        img_l, img_r = stereo_vision.preprocess_stereo_image(img_double, img_double.shape[0], img_double.shape[1])
        ...
        if key == ord('q'):
            ...
            depth_map, points_3d = stereo_vision.run(img_l, img_r)
            if mask is not None:
                points_3d, depth_map, img_l = stereo_vision.mask_3d(mask, points_3d, depth_map, img_l)
            depth_map_img = depth_map / 128
            cv2.imshow("depth", depth_map_img)
            stereo_vision.draw_point_cloud(points_3d, depth_map, img_l)
